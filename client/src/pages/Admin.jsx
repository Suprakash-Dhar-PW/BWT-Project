...
export default function Admin() {
  ...
  const [modalSnapshotInfo, setModalSnapshotInfo] = useState([]); // Static snapshot data
  ...
  // 2. DYNAMIC DERIVATIONS (Must be before hooks that use them)
  ...
  const currentPos = useMemo(() => {
    if (!settings || !positions.length) return positions[0];
    return (
  ...
  const eligibleVoters = useMemo(() => {
    return members.filter(
      (m) =>
        m.is_voter && !m.is_admin && m.email !== "system-state@bwt.internal",
    ).length;
  }, [members]);
  ...
  // Sync Protocol Facades
  const refreshMembers = useCallback(
    async () => await syncSystem(true),
    [syncSystem],
  );
  ...
  const clearDashboardStats = useCallback(() => {
    useStore.setState({ votes: [] });
    console.log("[STATS] Local tally buffers purged.");
  }, []);
    ...
    startTransition(() => {
      useStore.setState({
        ...
        members: members.map((m) =>
          !m.is_admin
            ? { ...m, is_voter: false, is_nominee: false, eliminated: false }
            : m,
        ),
      });
    });
  }, [settings, positions, members]);

  // 3. PERSISTENT TIE RECOVERY
  const fetchTieData = useCallback(async () => {
    if (!settings || !currentPos) return;

    try {
      const { data, error } = await supabase
        .from("election_results")
        .select("*")
        .eq("role", currentPos.name)
      ...
      if (data && data.length > 0) {
        console.log(
          "[PROTOCOL] Recovered static snapshot for",
          currentPos.name,
          "Round",
          settings.round_number,
        );
        const recoveringTally = {};
        data.forEach((row) => {
          recoveringTally[row.nominee_id] = row.votes;
        });
        setModalTally(recoveringTally);
        setModalSnapshotInfo(data);
        setShowManualReview(true);
      }
    } catch (e) {
      console.error("Snapshot Recovery Error:", e);
    }
  }, [settings, currentPos]);

  const hasAutoOpenedForStatus = useRef(null);

  useEffect(() => {
    // FIX: Auto-open for manual review ONLY if we haven't handled this specific transition already.
    // This prevents annoying reopening during other admin actions (like role toggling).
    if (
      settings?.status === "MANUAL_REVIEW" &&
      !actionLoading &&
      hasAutoOpenedForStatus.current !== "MANUAL_REVIEW"
    ) {
      fetchTieData();
      hasAutoOpenedForStatus.current = "MANUAL_REVIEW";
    } else if (settings?.status !== "MANUAL_REVIEW") {
      // Reset tracker if we move away from manual review
      hasAutoOpenedForStatus.current = null;
    }
  }, [settings?.status, actionLoading, fetchTieData]);

  const validateElectionReady = useCallback((n_count, v_count) => {
    if (n_count === 0) {
      return {
        valid: false,
        message: "Cannot start voting: No nominees selected."
      };
    }
    if (v_count === 0) {
      return {
        valid: false,
        message: "Cannot start voting: No voters selected."
      };
    }
    return { valid: true };
  }, []);
  ...
  const fetchModalData = useCallback(async () => {
    ...
    try {
      // 1. Contextual Check: Try to fetch from STATIC SNAPSHOT first (immutability)
      const { data: snapshotData, error: snapshotErr } = await supabase
        .from("election_results")
        .select("nominee_id, votes, winner_name, winner_email")
        .eq("role", currentPos?.name)
      ...
      if (snapshotData && snapshotData.length > 0) {
        console.log(
          `[GOVERNANCE] Using STATIC SNAPSHOT for Round ${settings.round_number}`,
        );
        const staticTally = snapshotData.reduce((acc, row) => {
          acc[row.nominee_id] = row.votes;
          return acc;
        }, {});

        setModalTally(staticTally);
        setModalSnapshotInfo(snapshotData);
        return;
      }

      // 2. Fallback: LIVE QUERY (only if snapshot isn't available yet)
      const { data, error } = await supabase
        .from("votes")
        .select("nominee_id")
      ...
      setModalSnapshotInfo([]); // Clear snapshot if using live data
    } catch (err) {
      console.error("Modal Data Fault:", err);
    }
  }, [settings?.current_position_id, settings?.round_number, currentPos?.name]);
  ...
  const bulkToggle = useCallback(
    async (column, value) => {
      ...
      try {
        if (
          (column === "is_voter" || column === "is_nominee") &&
  ...
  const selectAllNominees = useCallback(async () => {
    ...
    const originalMembers = [...members];
    const newMembers = members.map((m) =>
      !m.is_admin ? { ...m, is_voter: true, is_nominee: true } : m,
    );
    ...
    try {
      if (
        ...
        await Promise.all([
          ...
          supabase
            .from("settings")
            .update({ status: "SETUP", round_number: 1, winner_locked: false })
            .eq("id", settings.id),
        ]);
      }
  ...
  const updateMember = useCallback(
    async (id, updates) => {
      ...
      try {
        ...
        const isVoterChange = updates.hasOwnProperty("is_voter");

        // NOTE: Destructive reset removed to allow list modifications during tie-breakers.
        // Tally snapshots from previous rounds remain static and valid evidence.
  ...
  const toggleVoter = useCallback(
    async (m) => {
      await updateMember(m.id, { is_voter: !m.is_voter });
    },
    [updateMember],
  );
  ...
  const toggleNominee = useCallback(
    async (m) => {
      await updateMember(m.id, { is_nominee: !m.is_nominee });
    },
    [updateMember],
  );
  ...
  const transferAdmin = useCallback(
    async (newAdmin) => {
      ...
      try {
        ...
        const { error: transferErr } = await supabase.rpc(
          "transfer_admin_role",
          {
        ...
        console.log(
          "Atomic Transfer Complete. New registry state:",
  ...
  const handleResetVoting = async () => {
    ...
    try {
      console.log("[RESET] Initiating full election reset protocol...");

      // PAUSE BACKGROUND SYNC
      useStore.setState({ syncPaused: true });

      // STEP 1: CLEAR FINALIZED STANDINGS (RESULTS)
      console.log("[RESET] Clearing election_results...");
      ...
      if (resultsError) {
        console.error("[RESET ERROR - RESULTS]", resultsError);
        throw resultsError;
      }
      console.log("[RESET] election_results cleared:", resultsData);

      // STEP 2: CLEAR GOVERNANCE OVERRIDES (MANUAL DECISIONS)
      console.log("[RESET] Clearing manual_decisions...");
      ...
      // STEP 3: CLEAR BALLOT BOXES (VOTES)
      console.log("[RESET] Clearing votes...");
      ...
      // STEP 4: CLEAR POSITION WINNERS
      console.log("[RESET] Clearing position winners...");
      ...
      // STEP 5: RESET MEMBER REGISTRY
      const { error: memError } = await supabase
        .from("members")
        .update({ is_voter: false, is_nominee: false, eliminated: false })
        .eq("is_admin", false);

      if (memError) throw memError;

      // STEP 6: INITIALIZE ELECTION CLOCK
      ...
      console.log("[RESET] Atomic DB purge completed successfully.");
      ...
      // STEP 8: RESUME TRANSMISSION
      useStore.setState({ syncPaused: false });

      alert("System Reset Complete: Engine initialized to SETUP state.");
      setShowResetConfirm(false);
    } catch (err) {
      console.error("[PROTOCOL ERROR]", err);
      setErrorMsg(`System failure during reset: ${err.message}`);
      alert("Reset failed. Please retry transmission.");
      useStore.setState({ syncPaused: false });
    } finally {
      setActionLoading(false);
    }
  ...
  const updateSettings = useCallback(
    async (updates) => {
      ...
      setActionLoading(true);
      try {
        const recordId = settings?.id || 1;
        console.log(`[PROTOCOL] Persisting settings update for ID ${recordId}...`);
        ...
        // Double internal sync to prevent race conditions during navigation
        useStore.setState({ settings: { ...settings, ...updates, id: recordId } });
  ...
  const startElection = useCallback(async () => {
    const { valid, message } = validateElectionReady(nomineesCount, eligibleVoters);
    if (!valid) {
      setErrorMsg(message);
      return;
    }
    ...
    try {
      ...
      // 🛰️ PROTOCOL BROADCAST: Force Voter Hub to detect phase shift immediately
      const channel = supabase.channel("live-election");
      await channel.send({
        type: "broadcast",
        event: "START_VOTING",
        payload: { position_id: activePosId }
      });
      console.log("[PROTOCOL] Phase Activation Broadcasted.");
    } catch (err) {
  ...
  const stopVoting = useCallback(async () => {
    ...
    try {
      ...
      const winners = Object.entries(tally).filter(
        ([id, count]) => count === maxVotes,
      );
      ...
      if (!isTie && winners.length === 1 && maxVotes > 0) {
        console.log(
          "[PROTOCOL] Majority detected. Bypassing Governance Phase.",
        );

        // ALWAYS SNAPSHOT BEFORE FINALIZING: Capture round-level standings first
        const snapshotRows = Object.entries(tally).map(([nominee_id, count]) => {
          const m = members.find((mem) => mem.id === nominee_id);
          return {
            role: currentPos?.name,
            round_number: settings.round_number || 1,
            nominee_id,
            winner_name: m?.name || "Unknown Candidate",
            winner_email: m?.email || "unknown@system",
            votes: count,
            is_tie: false,
        ...
        if (snapshotRows.length > 0) {
          await supabase.from("election_results").insert(snapshotRows);
        }
        ...
        console.log(
          "[PROTOCOL] Tie detected or no votes cast. Entering Governance Phase.",
        );

        // SNAPSHOT: Store current result as static evidence
        const snapshotRows = Object.entries(tally).map(([nominee_id, count]) => {
          const m = members.find((mem) => mem.id === nominee_id);
          return {
            role: currentPos?.name,
  ...
  const startAnotherRound = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `[PROTOCOL] Clearing position-specific votes for Round ${nextRound} reset.`,
      );
      await supabase
        .from("votes")
        .delete()
  ...
  }, [settings, nomineesCount, eligibleVoters, updateSettings, syncSystem, actionLoading, validateElectionReady]);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20 mt-8 font-sans">
          ...
          <div className="border border-slate-200 rounded-lg p-4 bg-white shadow-sm">
            ...
            <div className="space-y-4 border-t border-slate-100 pt-3">
              {settings?.status === "SETUP" && (
                <button
                  disabled={actionLoading || nomineesCount === 0 || eligibleVoters === 0}
                ...
                >
                  <Play
                    className={cn("w-4 h-4", actionLoading && "animate-spin")}
                  />
              ...
              {(settings?.status === "MANUAL_REVIEW" || showManualReview) && (
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  <div className="p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100">
                    <h4 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-2">
                      Governance Phase
                    </h4>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight mb-3 opacity-60">
                      Round {settings.round_number} Tally Ready
                    </p>
                    <button
                      disabled={actionLoading}
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 p-2 bg-white border border-slate-200 rounded-lg shadow-sm">
            <div className="flex flex-wrap gap-1.5">
              <button
                ...
                onClick={() => bulkToggle("is_voter", true)}
              ...
              <button
                disabled={actionLoading || isVotingActive}
                onClick={() => bulkToggle("is_voter", false)}
                className="px-2.5 py-1.5 bg-slate-50 border border-slate-200 text-slate-600 rounded text-[10px] font-semibold uppercase tracking-widest hover:bg-slate-100 transition disabled:opacity-30"
              >
                Deselect All Voters
              </button>
            ...
            >
              {actionLoading ? "Resetting..." : "Reset Voting"}
            </Button>
          </div>
      ...
      <AlertModal
        ...
        message="Are you sure you want to reset all voting data? This will purge all votes, standings, and member statuses. This action cannot be undone."
            ...
            >
              <div className="p-10">
                ...
                <div className="space-y-6 mb-10">
                  ...
                  {nomineesCount !== prevNomineeCount &&
                    prevNomineeCount > 0 && (
                      <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-3 mb-6">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-amber-700 font-medium">
                            Previous results invalidated. Please conduct voting
                            again.
                          </p>
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 max-h-[300px] overflow-y-auto pr-2">
                    {Object.entries(modalTally)
                      .sort((a, b) => b[1] - a[1])
                      .map(([mId, vCount]) => {
                        const liveMember = members.find((mem) => mem.id === mId);
                        const snapshotMember = modalSnapshotInfo.find(
                          (s) => s.nominee_id === mId,
                        );

                        // If member is no longer in live list, use snapshot data
                        const m = liveMember || {
                          id: mId,
                          name: snapshotMember?.winner_name || "Unknown Candidate",
                        };

                        const maxV = Math.max(...Object.values(modalTally), 0);
                        const topCandidates = Object.entries(modalTally).filter(
                          ([id, count]) => count === maxV,
                        );
                        const isTie = topCandidates.length > 1;
                        const isNaturalWinner =
                          !isTie && mId === topCandidates[0]?.[0];
                        const isSelected =
                          manualSelection === mId ||
                          (isNaturalWinner && !manualSelection);
                        const isTiedCandidate =
                          isTie && vCount === maxV && maxV > 0;

                        return (
                          <button
                            key={mId}
                            disabled={!isTie && !isNaturalWinner}
                            onClick={() => isTie && setManualSelection(mId)}
                            className={cn(
                              "flex items-center justify-between p-4 rounded-2xl border transition-all text-left",
                              isSelected
                                ? "bg-indigo-600 border-indigo-600 text-white shadow-xl shadow-indigo-100"
                                : "bg-white border-slate-50 hover:border-indigo-100 text-slate-900",
                              isTiedCandidate &&
                                !isSelected &&
                          ...
                          >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <span
                                className={cn(
                                  "text-[9px] font-bold uppercase opacity-60",
                ...
                <div className="space-y-4">
                  ...
                  {(() => {
                    ...
                    if (!isTie && topCandidates.length === 1) {
                      ...
                      const staticWinner = modalSnapshotInfo.find(
                        (s) => s.nominee_id === winId,
                      );
                      const winName =
                        members.find((m) => m.id === winId)?.name ||
                        staticWinner?.winner_name ||
                        "Unknown Candidate";
                      return (
                    ...
                    if (isTie) {
                      return (
                        <div className="grid grid-cols-2 gap-4">
                          <button
                            ...
                            onClick={() => {
                              const maxVotes = Math.max(
                                ...Object.values(modalTally),
                                0,
                              );
                              const naturalWinnerId = Object.entries(
                                modalTally,
                              ).find(([id, count]) => count === maxVotes)?.[0];
                          ...
                          >
                            <AlertTriangle className="w-4 h-4" /> Apply Manual
                            Override
                          </button>
                          <button
                            disabled={actionLoading || nomineesCount === 0 || eligibleVoters === 0}
                            ...
                            className="h-14 bg-indigo-50 text-indigo-600 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-100 disabled:opacity-30 transition-all flex items-center justify-center gap-2"
                          >
                            <RefreshCw className={cn("w-4 h-4", actionLoading && "animate-spin")} /> Run Runoff Round
                          </button>
                        </div>
                      );
                    }
                    ...
                    return (
                      <div className="p-8 text-center bg-slate-50 rounded-2xl border border-slate-100">
                        <p className="text-xs font-bold text-slate-400">
                          No valid votes recorded for this round.
                        </p>
                      </div>
                    );
                  })()}
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            {!isMemberAdmin && (
              <div className="flex flex-col items-center">
                ...
                <button
                  ...
                  onClick={async () => await toggleVoter(m)}
                  className={cn(
                    "relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2",
                    m.is_voter ? "bg-emerald-500" : "bg-slate-200",
                ...
                >
                  <span
                    className={cn(
                      ...
                      m.is_voter ? "translate-x-5" : "translate-x-0",
                    )}
                  />
                </button>
              </div>
            ...
            {!isMemberAdmin && (
              <div className="flex flex-col items-center">
                ...
                <button
                  disabled={isLocalLoading || isWinner || isVotingActive}
                  onClick={async () => await toggleNominee(m)}
                  className={cn(
          ...
          <div className="flex items-center gap-1">
            {!isMemberAdmin && !isWinner && (
              ...
              >
                <Shield
                  className={cn(
                    "w-5 h-5 group-hover:fill-indigo-500",
                    pendingId === m.id && "animate-spin",
                  )}
                />
              </button>
            )}
